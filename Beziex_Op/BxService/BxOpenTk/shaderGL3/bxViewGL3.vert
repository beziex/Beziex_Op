...
in vec3   vertVertexUV;
in float  vertInstanceID;
...
{
    int  division = 20;
    int  subNo    = int( surfaceNo / division );
    int  numSub   = int( ( numSurface + division - 1 ) / division );
    int  subMod   = int( mod( surfaceNo, division ) );

    float  u = ( ( subMod * 80 ) + index + 0.5 ) / ( 80 * division );
    float  v = ( subNo + 0.5 ) / numSub;

    vec4  val = texture( vtf, vec2( u, v ) );
